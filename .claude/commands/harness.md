...
하나의 Step으로 처리 가능한 규모:
- 생성/수정 파일 수: 5개 이하
- 추정 코드 라인: 200줄 이하
- 도입되는 새 외부 패키지: 2개 이하

반드시 쪼개야 하는 신호:
- "~도 하고 ~도 한다"는 작업 설명이 나올 때
- DB 스키마 변경과 API 구현을 동시에 다룰 때
- 새 외부 서비스 연동(OAuth, 결제 등)과 UI를 한 step에 묶을 때
- 한 step의 실패 원인이 2개 이상의 독립된 원인에서 올 수 있을 때

묶어도 되는 기준:
- 같은 파일 내 CRUD 4개 메서드
- 타입 정의와 그 타입을 쓰는 유틸 함수 (타입이 1개 파일이면)
- 테스트 픽스처와 첫 번째 테스트
...
`summary`는 다음 Claude 세션이 이 한 줄만 보고 이전 산출물을 파악할 수 있어야 한다.

**좋은 패턴:**
- `src/lib/db.ts 생성 — Prisma Client 싱글톤 + connectDB()/disconnectDB() 익스포트`
- `app/api/auth/[...nextauth]/route.ts 추가 — Google OAuth 프로바이더, JWT 세션 전략`
- `components/TaskCard.tsx, TaskList.tsx 생성 — props 타입: Task 인터페이스 (types/task.ts)`

**피해야 할 패턴:**
- `인증 기능 구현 완료` → 어떤 파일이 생겼는지, 어떤 인터페이스가 익스포트되는지 알 수 없음
- `테스트 통과` → 산출물이 없음

**포함해야 할 정보:**
1. 생성/수정된 핵심 파일 경로
2. 외부에서 사용하는 주요 함수/클래스/타입 이름
3. 다음 step이 알아야 할 결정 (예: `userId는 string이 아닌 UUID 타입`)
